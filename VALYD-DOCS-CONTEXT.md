# Valyd Documentation — Full Context (for regenerating docs.valyd.work)

> This single file bundles ALL existing Valyd documentation content plus a
> structural map, so an AI can recreate / rewrite the docs from one source.
> No styling — raw context only. Every markdown section below is the CURRENT
> published text, embedded verbatim. Interactive React-only pages are listed
> with a description since they have no markdown source.
>
> Live docs site: https://docs.valyd.work  (Vite + React + TS + Tailwind + shadcn-ui)
> Source repo: /var/www/pollus_main_servers/docs

---

## 0. What Valyd is (platform overview)

Valyd is an identity + verification platform. The docs cover two products plus
supporting surfaces:

1. **Login with Valyd (Valyd ID / TPSSO)** — OAuth2 / OIDC single sign-on. A
   third-party app redirects a user to Valyd, the user consents, and the app
   receives verified profile data, professional licenses, and identity
   verification status. Base: `https://idp.valyd.work/api/auth/tpsso`.

2. **Valyd Verify** — identity verification APIs. A 2×2 model:
   **Account (Managed by Valyd)** vs **Non-account (Fresh)**, each available
   **Hosted** (Valyd renders the capture page) or **Core APIs** (you call REST
   with your own UI). Base: `https://idp.valyd.work`. Services: id_verification,
   liveness, face_match, age, credential (professional license).

3. **Developer Portal / Console** (`https://dev.valyd.work`) — where developers
   sign up, create projects/apps, get `client_id`/`client_secret` (for Login) and
   an App API key + workflows (for Verify), and manage billing.

4. **SDKs** — `valyd-idp-sdk` (Login/OIDC), `valyd-verify-sdk` / `@valyd/sdk`
   (Verify), `valyd-verify-js` (browser modal for hosted verification).

5. **Interactive / product pages** — EVV, Anti-Spoof (liveness) demo, Hosted KYC,
   Verify License, AI Agents / MCP integration.

Data-sharing rule (core principle, repeated across Verify docs): **Account APIs
return proofs only** (a pseudonym, `id_verified`, verified license badges, age
bands) and **never** raw KYC (legal name, DOB, document images). Raw account
attributes are released only through the **consent Core API**, where the user
approves release in their Valyd app and values are sealed to the caller's public
key. **Non-account (Fresh) APIs return the captured raw data as-is.**

Response envelope (every Verify API): `{ success, data, error }` — `error` is
present only when `success` is false.

---

## 1. Documentation structure (site map)

### A. Valyd ID / Login docs (path: /docs/*)
- overview — Third-Party SSO API intro + 6-step OAuth flow
- quick-start — end-to-end login integration
- authentication — token exchange, access/refresh tokens
- oidc — OpenID Connect specifics
- scopes — available scopes and what each grants
- login-sessions — login session handling
- create-project — registering an app in the Developer Portal
- endpoints — full IdP endpoint reference (largest doc, ~759 lines)
- errors — error codes
- changelog

### B. Valyd Verify docs (path: /verify/*)
- intro — Verify overview + the 2×2 model + services + envelope
- modes — Hosted vs Core APIs decision tree
- managed — Account (Managed by Valyd) API type
- standalone — Non-account (Fresh) / Core APIs (~505 lines)
- hosted — Hosted sessions flow (~486 lines)
- quickstart — first Verify call
- console — Developer Console (keys, workflows, webhooks, billing)
- webhooks — webhook payloads + signature verification
- statuses — verification status values
- api-reference — Verify REST reference
- sdk — Verify SDK usage (~296 lines)

### C. Interactive React-only pages (no markdown source — describe from product knowledge)
- Landing / Index — docs home
- IdApiRef — interactive Valyd ID API reference (OpenAPI-driven, "try it")
- VerifyApiRef — interactive Verify API reference
- TryApis — live API playground
- EvvPage — Electronic Visit Verification integration guide
- AntiSpoofPage — liveness / anti-spoofing demo (Login with Valyd → liveness)
- HostedKycPage — hosted KYC walkthrough
- VerifyLicensePage — professional license verification guide
- Agents — machine-readable docs index for AI agents
- McpDocs — MCP (Model Context Protocol) server integration for AI agents

---

## 2. FULL EXISTING DOCUMENTATION CONTENT (verbatim)

The remainder of this file is the complete current text of every markdown doc,
each under a clear separator showing its source path and live URL.


===================================================================
### FILE: docs/overview.md   (live: https://docs.valyd.work/docs/overview)
===================================================================
...
===================================================================
### FILE: docs/quick-start.md   (live: https://docs.valyd.work/docs/quick-start)
===================================================================
...
===================================================================
### FILE: docs/authentication.md   (live: https://docs.valyd.work/docs/authentication)
===================================================================
...
===================================================================
### FILE: docs/oidc.md   (live: https://docs.valyd.work/docs/oidc)
===================================================================
...
===================================================================
### FILE: docs/scopes.md   (live: https://docs.valyd.work/docs/scopes)
===================================================================
...
===================================================================
### FILE: docs/login-sessions.md   (live: https://docs.valyd.work/docs/login-sessions)
===================================================================
...
===================================================================
### FILE: docs/create-project.md   (live: https://docs.valyd.work/docs/create-project)
===================================================================
...
===================================================================
### FILE: docs/endpoints.md   (live: https://docs.valyd.work/docs/endpoints)
===================================================================
...
> **Authorization codes are bound to the client they were issued to**, are single-use, and expire
> **2 minutes** after issue. Exchange the code as soon as your callback receives it. A code issued
> for one application can never be redeemed by another.
...
| `redirect_uri` | string | Recommended | The **exact** `redirect_uri` used at `/authorize`. Validated when supplied; will become required in a future release |
...
and the legitimate session stop working, and the user simply signs in again.
...
===================================================================
### FILE: docs/errors.md   (live: https://docs.valyd.work/docs/errors)
===================================================================
...
===================================================================
### FILE: docs/changelog.md   (live: https://docs.valyd.work/docs/changelog)
===================================================================
...
===================================================================
### FILE: verify/intro.md   (live: https://docs.valyd.work/verify/intro)
===================================================================
...
===================================================================
### FILE: verify/modes.md   (live: https://docs.valyd.work/verify/modes)
===================================================================
...
===================================================================
### FILE: verify/managed.md   (live: https://docs.valyd.work/verify/managed)
===================================================================
...
===================================================================
### FILE: verify/standalone.md   (live: https://docs.valyd.work/verify/standalone)
===================================================================
...
===================================================================
### FILE: verify/hosted.md   (live: https://docs.valyd.work/verify/hosted)
===================================================================
...
===================================================================
### FILE: verify/quickstart.md   (live: https://docs.valyd.work/verify/quickstart)
===================================================================
...
===================================================================
### FILE: verify/console.md   (live: https://docs.valyd.work/verify/console)
===================================================================
...
===================================================================
### FILE: verify/webhooks.md   (live: https://docs.valyd.work/verify/webhooks)
===================================================================
...
===================================================================
### FILE: verify/statuses.md   (live: https://docs.valyd.work/verify/statuses)
===================================================================
...
===================================================================
### FILE: verify/api-reference.md   (live: https://docs.valyd.work/verify/api-reference)
===================================================================
...
===================================================================
### FILE: verify/sdk.md   (live: https://docs.valyd.work/verify/sdk)
===================================================================
