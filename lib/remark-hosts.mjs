...
// drive the output.
//
//   NEXT_PUBLIC_DOCS_URL  NEXT_PUBLIC_IDP_URL  NEXT_PUBLIC_DEV_URL  NEXT_PUBLIC_MCP_URL
...
function walk(node) {
  ...
  // MDX JSX elements (<a href="https://idp.valyd.work/...">) keep their props in
  // `attributes`, not `children` — without this the home page's raw JSX links
  // shipped .work hosts on .vip/.id builds and tripped check-hosts.
  if (Array.isArray(node.attributes)) {
    for (const attr of node.attributes) {
      if (typeof attr?.value === 'string' && attr.value.includes('valyd.work')) {
