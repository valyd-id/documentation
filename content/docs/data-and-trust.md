...
| [Standalone](/verifications/unique-human) check results | Returned to your system only; nothing is added to a Valyd account. Raw KYC fields in a hosted decision are released only after the required ID, liveness, and face-match gates pass — until then they remain encrypted. |
| Account-connected results | The account APIs return **proofs only**, never raw account KYC ([details](/verifications)). |
