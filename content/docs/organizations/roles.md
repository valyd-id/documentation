...
**developer** builds apps; a **member** is the workforce and only signs into your apps by face.
...
| `member` | No | Yes | The **workforce**. Does **not** see the organization at all — a member exists only to sign into the apps assigned to them, **by face**. Face-keyed: the membership binds to a real Valyd identity when they scan their face, and that identity is how they log in. |
