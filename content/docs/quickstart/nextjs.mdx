...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ValydError.code: access_denied (user cancelled), invalid_grant, issuer_mismatch, ...
...
- **`access_denied`** — the user pressed **Cancel** on the consent screen (or isn't assigned to
  a private org app); Valyd redirects back with `?error=access_denied` and `handleCallback()`
  throws it.
- **`pkce_required`** — `getAuthorizationUrl()` was called without a `codeChallenge`; keep using
  `createAuthorizationRequest()` (PKCE S256 is required, `@valyd/sdk` ≥ 1.12).
