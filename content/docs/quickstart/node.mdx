---
title: Node.js (Express) quickstart — Login with Valyd
description: 'Add Login with Valyd to an Express app in one file with @valyd/sdk: authorization request, server-side transaction, verified callback.'
---
...
**What you'll build:** a single-file Express app where "Login with Valyd" returns the user's
...
app.get("/", (_req, res) => res.type("html").send('<a href="/login">Login with Valyd</a>'));
...
Open http://localhost:8080 and click **Login with Valyd**. A successful callback returns the
