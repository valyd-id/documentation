...
<Tabs items={['Hosted by Valyd', 'Standalone checks']}>
<Tabs.Tab>
...
Your own UI calls Verify directly. It still begins with Connect Valyd (to get the token); KYC is a redirect to Valyd (no KYC API); the license and face+location run as standalone check calls.
...
// Building your own capture UI (standalone path)? There is no Valyd browser
...
SDKs (install):
- Server (Node 18+):  npm i @valyd/sdk@^1.10.2    // valyd.auth (OIDC) + valyd.verify (checks)
...
Flow B — Standalone checks (your own UI):
