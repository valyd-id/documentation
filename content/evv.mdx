...
app.get("/evv/callback", async (req, res) => {
  ...
  if (!v.id_verified) {                                  // missing KYC → run it on Valyd's hosted page
    const s = await valyd.verify.sessions.create({ workflowId: KYC_WORKFLOW_ID, valydAccessToken: accessToken, redirectUrl: "https://app/evv" });
    return res.redirect(s.url);
  }
