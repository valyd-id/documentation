...
description: Prove the right clinician reached the right home — with verified identity, a live medical license, a face match, and geolocation. Built on Managed Identity, so clinicians verify once and reuse that identity on every visit.
---
...
Prove the **right clinician** reached the **right home** — with verified identity, a live medical license, a face match, and geolocation. Built on Managed Identity, so clinicians verify once and reuse that identity on every visit — each visit still gets a fresh face + location check.
...
<Cards>
  ...
  <Cards.Card title="Medical license" href="/verifications/managed">
    Checked live against the state board, then stored on the account and re-checked over time.
  </Cards.Card>
  <Cards.Card title="Face at the door" href="/verifications/managed">
    A live selfie matched against the clinician's stored Valyd face vector.
  </Cards.Card>
  <Cards.Card title="Location" href="/verifications/managed">
...
- **Verify API key + workflow** — in the **same portal**, set up your app's verification capability (surfaced as a Verify "project") → copy the `API key` (`vrf_…`, shown once) + `webhook secret`. Then **New workflow → "Home Health · EVV"** (pre-selects ID, liveness, face match, license & location) → copy its `workflow_id`.
...
# browser: none — the face + GPS are captured on Valyd's hosted session page
...
**Both modes start the same way — "Connect Valyd".** The clinician logs in with Valyd (OAuth), and your server gets their access token via `exchangeCode`. That token is what identifies the person, gates KYC, and unlocks license/identity reuse. EVV runs on **Managed by Valyd** — the sensitive checks (ID/KYC, license, face match, location) run on Valyd's hosted session with that token, never as self-serve direct calls.
</Callout>
...
Split the flow: **onboard the clinician once** (KYC + license), then run a **lean per-visit session** (face + location) on every visit. Both halves are Managed by Valyd — the onboarding gate is a redirect to Valyd, and each visit is a hosted session bound to the clinician's token. No sensitive check is a direct call.
...
app.get("/evv/callback", async (req, res) => {
  ...
  // 3) The visit — a Managed session bound to the token runs the face + location checks on Valyd's
  //    hosted page. ID/KYC + license are reused from the account, so a returning clinician does
...
```javascript filename="Browser — Connect Valyd, then the hosted visit"
// No browser SDK — both "Connect Valyd" and the hosted visit are redirects.
...
SDKs (install):
- Server (Node 18+):  npm i @valyd/sdk@^1.10.4    // valyd.auth (OIDC) + valyd.verify (checks)
...
- KYC + license are ONE-TIME onboarding steps (redirect to Valyd — they run on the hosted page, never as
  direct calls). Do NOT put a KYC/license button on every visit. The recurring visit is a lean Managed
  session with just face_match + location (ID/license reused from the account).
- The recurring visit is a sessions.create({ workflowId, valydAccessToken, metadata }) call; send the
  clinician to session.url and read sessions.decision(id). No self-serve direct/standalone check calls.
...
Flow B — Onboard once, then a lean per-visit session (still all Managed by Valyd):
...
3. Each visit: create a lean Managed session (face + location only; id/license reused from the account):
...
<Callout type="info">
Because EVV runs on **Managed Identity**, the first visit does full KYC + license; every visit after is **just a face + location check**. A stored license is never served past its `expire_at` date — an expired credential fails the check and must be re-verified — and identity/KYC live on Valyd — your app stores proofs, not documents.
</Callout>
