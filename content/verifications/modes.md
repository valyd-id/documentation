...
## Agent Quick-Start
- Source URL: https://docs.valyd.work/verifications/modes
...
[Account (Managed by Valyd)](/verifications/managed).
...
IF you need an end-user KYC flow with live camera/ID + selfie capture, and you do NOT want to build the capture UI:
  ...
  → Next: create a Workflow (get workflow_id) and configure a webhook in the Console (https://docs.valyd.work/verifications/console), then create a session (https://docs.valyd.work/verifications/quickstart).
...
IF you want to call individual capabilities server-to-server from your backend with your own UI and data, and want a synchronous result:
  ...
  → Next: get an App API key from the Console (https://docs.valyd.work/verifications/console), then call the endpoint (https://docs.valyd.work/verifications/quickstart).
