...
description: A ready-to-go demo — let a user connect with Valyd and share their verified details, then run a passive-liveness check that flags presentation attacks like photos, masks, replays, and deepfakes.
---
...
A ready-to-go demo: let a user connect with Valyd and share their verified details, then run a passive-liveness check that flags presentation attacks — photos, masks, replays, deepfakes. Copy the credentials and the snippet below and you're running in minutes.
...
## Server (Node) — connect, share details, verify face
...
// 1) Connect button → send the user to Valyd to authorize.
...
<button onclick="location.href='/login'">Connect with Valyd</button>
...
If you also want to know **who** the live person is, add the anti-spoof **+ identity** check to your workflow. It runs the same anti-spoof pipeline and, only when it passes, resolves the proven-live face against the global Valyd gallery — returning a **stable `valyd_` uuid**. The same face always maps to the same uuid, so this is duplicate-account / sybil detection in a single request. See [Anti-spoof + identity](/verifications/unique-human/antispoof#post-apiv2antispoofidentity--anti-spoof--identity) and [Face uniqueness](/verifications/unique-human/face-uniqueness).

## Next steps

- [Liveness API](/verifications/unique-human/antispoof) · [Uniqueness API](/verifications/unique-human/face-uniqueness)
- [Run a verification](/verifications/quickstart)
- [Reusable Verification docs](/verifications)
